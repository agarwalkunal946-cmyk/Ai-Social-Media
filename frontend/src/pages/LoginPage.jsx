...
import { ArrowRight, Eye, EyeOff, Github, KeyRound, Mail, Moon, Sun, Zap } from "lucide-react";
...
export function LoginPage() {
  ...
  }, [firebaseUser, backendUser, navigate]);
      ...
      <div className="relative z-10 w-full max-w-[480px]">
        <motion.div initial={{ opacity: 0, y: 24 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5 }}>
          ...
          <div className="rounded-2xl border border-white/[0.06] bg-slate-900/60 p-8 shadow-ambient backdrop-blur-2xl">
            ...
            <div className="space-y-4">
              ...
              <div className="flex items-center justify-end">
                <button
                  className="text-xs font-medium text-slate-400 transition hover:text-neon disabled:cursor-not-allowed disabled:opacity-60"
              ...
              <Button className="w-full gap-2" onClick={() => wrap("login", () => loginWithEmail(email, password))} disabled={loading === "login"}>
          ...
          <p className="mt-6 text-center text-sm text-slate-500">
            ...
            <Link to="/signup" className="font-medium text-neon transition hover:text-cyan-300">Create account</Link>
          </p>
        </motion.div>
      </div>
    </div>
